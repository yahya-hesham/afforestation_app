...
class AddOperationInitial extends AddOperationState {}
class AddOperationLoading extends AddOperationState {}
class AddOperationSuccess extends AddOperationState {}
class AddOperationFailure extends AddOperationState {
  final String errorMessage;
  const AddOperationFailure(this.errorMessage);
}

class AddOperationCubit extends Cubit<AddOperationState> {
  AddOperationCubit() : super(AddOperationInitial());

  void addNewOperation({required String operationName, required String date}) async {
    emit(AddOperationLoading());
    await Future.delayed(const Duration(seconds: 1));
    emit(AddOperationSuccess());
  }
}
...
class _AddNewOperationScreenState extends State<AddNewOperationScreen> {
  ...
  final TextEditingController _dateController = TextEditingController(text: '2026-07-17'); 

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => AddOperationCubit(),
      child: Scaffold(
        ...
        body: BlocConsumer<AddOperationCubit, AddOperationState>(
          listener: (context, state) {
            if (state is AddOperationSuccess) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('تم إضافة عملية التشجير بنجاح! 🎉'),
                    ...
                    children: [
                            ...
                            : () {
                                if (_formKey.currentState!.validate()) {
                                  if (_selectedPlantName == null) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(content: Text('برجاء اختيار اسم النبات أولاً')),
                                    );
                                    return;
                                  }
                                  context.read<AddOperationCubit>().addNewOperation(
                                        operationName: _selectedPlantName!,
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(color: const Color(0xFF56B76C).withValues(alpha: 0.15)),
                        ),
                        child: const Row(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
